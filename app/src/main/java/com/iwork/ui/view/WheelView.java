...
public class WheelView extends ScrollableView implements OnClickListener {
    ...
    public static final int TEXT_GRAVITY_RIGHT = -1;
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        this.splitLineWidth=a.getFloat(R.styleable.wheel_splitLineWidth, -1);
        ...
        if (this.splitLineWidth>0){
            this.linePaint.setStrokeWidth(splitLineWidth);
        }
    ...
    private void drawSplitLine(Canvas canvas) {
        ...
        canvas.drawLine(0, this.thirdRectTop-margin, getMeasuredWidth(), this.thirdRectTop-margin, linePaint);
        canvas.drawLine(0, this.thirdRectBottom+margin, getMeasuredWidth(), this.thirdRectBottom+margin, linePaint);
    }
    ...
    /**
     * 设置滚轮文字的gravity，该算法暂支持居中和靠右两种
     */
    private float computeTextX(float maxWidth, int px, float textWidth) {
        if (this.textGravity == TEXT_GRAVITY_CENTER)
    ...
    public int getStartValue() {
        return this.adapter.getStartValue();
    }
}
